...
class ImageClassificationEvaluator:
    ...
    def predict(self):
        ...
        inference_outputs_list_warmup = []
        inference_outputs_list_timing = []
        ...
        for i in range(10):
            ...
            inference_outputs_list_warmup.append(output)
        images_for_warmup = len(inference_outputs_list_warmup) * self.data_reader.batch_size
        
        print('######## Start to timing inference #########')    
        ...
        while True:
            ...
            if not inputs:
                break
            output = session.run(None, inputs)
            inference_outputs_list_timing.append(output)
        T2 = time.perf_counter()
        images_for_timing = len(inference_outputs_list_timing) * self.data_reader.batch_size
        
        print('############# Finish inference #############')
        print('Inferenced %d images for warmup' %images_for_warmup)
        print('Inferenced %d images for timing' %images_for_timing)
        print('Timing used %d ms' %((T2-T1)*1000))
        print('Timing throughput: %d fps' %(images_for_timing/(T2-T1)))
        self.prediction_result_list = inference_outputs_list_warmup + inference_outputs_list_timing
...
def convert_model_batch_to_dynamic(model_path):
    ...
    input_shape = inputs[0].type.tensor_type.shape
    input_dim = input_shape.dim
    output_name = outputs[0].name
    output_shape = outputs[0].type.tensor_type.shape
    output_dim = output_shape.dim
    if not input_dim[0].dim_param and not output_dim[0].dim_param:
        input_dim[0].dim_param = 'N'
        #model = onnx.shape_inference.infer_shapes(model)
        #model_name = model_path.split(".")
        #model_path = model_name[0] + "_dynamic.onnx"
        #onnx.save(model, model_path)
        output_dim[0].dim_param = 'N'
        model = onnx.shape_inference.infer_shapes(model)
        model_name = model_path.split(".")
        model_path = model_name[0] + "_dynamic.onnx"
        onnx.save(model, model_path)
    return [model_path, input_name, output_name]
...
if __name__ == '__main__':
    ...
    os.environ["ORT_TENSORRT_ENGINE_CACHE_ENABLE"] = "0"  # Enable engine caching
    execution_provider = ["TensorrtExecutionProvider"]

    # Convert static batch to dynamic batch
    [new_model_path, input_name, output_name] = convert_model_batch_to_dynamic(model_path)
    ...
    print('Calibration dataset size: %d images' %calibration_dataset_size)
    print('Prediction dataset size: %d images' %prediction_dataset_size)
